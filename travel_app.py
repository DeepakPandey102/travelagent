...
# ─────────────────────────────────────────────
    ...
    if st.button("🚀 Generate Full Travel Plan", use_container_width=False):
        ...
        with st.spinner("🔍  Researching attractions and admission prices..."):
            ...
            try:
                res = research_agent.run(
                    f"Top attractions + admission prices in {dest_city}. {activities}. {special_notes}. Currency: {sym}.",
                    stream=False)
                st.session_state.research_text = res.content
            except Exception as e:
        ...
        with st.spinner("🏨  Evaluating hotels and dining options..."):
            ...
            try:
                res = hotel_agent.run(f"Hotels and restaurants in {dest_city} for {hotel_stars}, budget tier {budget_tier}. Prices in {sym}.", stream=False)
                st.session_state.hotel_text = res.content
            except Exception as e:
...
3. Transit style: {transit_style}. Cabin: {flight_class}. Theme: {theme}."""
            ...
            try:
                res = planner_agent.run(
                    f"{days_in}-day plan for {dest_city}. Currency {sym}. Budget {sym}{max_budget}.\nActivities: {st.session_state.research_text}\nLodging: {st.session_state.hotel_text}",
                    stream=False)
                st.session_state.itinerary_text = res.content
            except Exception as e:
